...
const Navbar = ({setSortedArray, setSearchedArray, usersList, changeTheStyle ,userStyle, title }) => {
 ...
 const onChangeHandler=(e)=>{
  ...
  const newArray = usersList.filter(person =>
    ...
    (person.name.first+" "+person.name.last).toLowerCase().includes(e.target.value)
  );
  ...
  let icon
  userStyle?icon= list:icon=grid
  
  return (
    <div className="navbar">

   <div className="arrows">
    <div className="arrow" onClick={sortArrayAscending}><img src={ascending} alt="" /></div>
    <div className="arrow" onClick={sortArrayDescending}><img src={descending} alt="" /></div>
    </div>
    ...
    <div  onClick={changeStyle}><button className="btn"><img src={icon} alt="" /></button></div>
